        ...
        marked.use({
            renderer: {
                code(code, language) {
                    ...
                    let highlighted;
                    let detectedLang = highlightLang;
                    
                    if (self.isHighlightReady) {
                        try {
                            const result = hljs.highlightAuto(code);
                            highlighted = result.value;
                            detectedLang = result.language || highlightLang;
                        } catch (e) {
                    ...
                    // Re-check if HTML after detection
                    const isDetectedHtml = detectedLang === 'html' || detectedLang === 'xml' || detectedLang === 'htmlmixed';
                    const shouldShowPreview = isHtml || isDetectedHtml || self._isHtmlCode(code);
                    
                    const displayLabel = originalLabel || detectedLang;
                    const previewBtn = shouldShowPreview ? `<button class="preview-btn" data-code="${encodeURIComponent(code)}" onclick="renderer.showHtmlPreviewModal(this)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8z"/><circle cx="12" cy="12" r="3"/></svg>Preview</button>` : '';
