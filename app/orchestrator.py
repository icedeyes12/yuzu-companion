...
# ---------------------------------------------------------------------------
# Native tool-call helpers (DEPRECATED in v3.1.0)
# ---------------------------------------------------------------------------
# Native tool calling has been removed. The LLM now outputs inline /commands
# which are detected by StreamFilter and executed via execute_command().

# The following functions are kept for reference but are no longer used:
# - _parse_raw_tool_calls: was used to parse native tool_call from API response
# - _execute_tool_calls: was used to execute native tool_calls
# These have been replaced by the inline /command system.
...
def handle_user_message(user_message: str, interface: str = "terminal") -> str:
    ...
    # v3.1.0: Native tool calling removed - LLM uses inline /commands
